...
def create_image_lists(INPUT_DATA):
    ...
    while len(matched_result) < 1300: # 根据图像数量修改此处
        for sub_dir in sub_dirs[1:]:
            ...
            base_name1 = label_name + "/" + os.path.basename(file_list[random_number1 % length])  # 获取文件的名称
            base_name2 = label_name + "/" + os.path.basename(file_list[ random_number2 % length])

            s = base_name1+ ' ' + base_name2 + ' ' + '1'
            if(file_list[random_number1%length] != file_list[random_number2%length]) and (s not in matched_result):
                # 将当前类别的数据放入结果字典
        ...
        for i in range(length_of_dir):
            ...
            if ((length_of_dir-i+j-1) % length_of_dir):
                ...
                if file_list1 and file_list2:
                    base_name1 = class1_name + "/" + os.path.basename(file_list1[j % len(file_list1)])  # 获取文件的名称
                    base_name2 = class2_name + "/" + os.path.basename(file_list2[j % len(file_list2)])

                    # unmatched_result.add([class1_name, base_name1, class2_name, base_name2])
                    s = base_name1 + ' ' + base_name2 + ' ' + '0'
                    if(s not in unmatched_result):
                        unmatched_result.add(base_name1 + ' ' + base_name2 + ' ' + '0')
                    k = k + 1
    return unmatched_result, k

if __name__ == '__main__':
    # val_data path
    INPUT_DATA = '/home/Face_recognition/v1_pretraining_resnet18/data/val'

    result, k1 = create_image_lists(INPUT_DATA) # match image pair
    result_un, k2 = create_pairs(INPUT_DATA) # unmatch image pair

    # pair file output path
    file = open('/home/Face_recognition/v1_pretraining_resnet18/data/pairs.txt', 'w')
