...
def estimate_pose_norm_kpts(kpts0, kpts1, thresh=1e-3, conf=0.99999):
	"""
	使用RANSAC这种鲁棒的方法再次计算E
	"""
	if len(kpts0) < 5:
		return None

	# 超过threshold的认定为outliers
	# prob是essential matrix的confidence
	# 用RANSAC再对点进行筛选，再用8点匹配
	# mask中0表示outliers
	...
	for _E in np.split(E, len(E) / 3):
		n, R, t, mask_ = cv2.recoverPose(_E, kpts0, kpts1, np.eye(3), 1e9, mask=mask) # 通过预测的内点和e_hat得到预测R、t
		if n > best_num_inliers:
			best_num_inliers = n
			ret = (R, t[:, 0], mask.ravel() > 0) # ravel将数组拉为一维，R、t均为预测
