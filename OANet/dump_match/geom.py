import numpy as np
from dump_match.utils import loadh5
...
def get_episym(x1, x2, dR, dt):
    ...
    # Make homogeneous coordinates/齐次坐标
    x1 = np.concatenate([x1, np.ones((num_pts, 1))], axis=-1).reshape(-1, 3, 1) # [2000,2] -> [2000,3,1]
    x2 = np.concatenate([x2, np.ones((num_pts, 1))], axis=-1).reshape(-1, 3, 1) # [2000,2] -> [2000,3,1]

    # Compute Fundamental matrix
    dR = dR.reshape(1, 3, 3)
    dt = dt.reshape(1, 3)
    F = np.repeat(np.matmul(np.reshape(np_skew_symmetric(dt), (-1, 3, 3)), dR).reshape(-1, 3, 3), num_pts, axis=0) # [2000,3,3]

    x2Fx1 = np.matmul(x2.transpose(0, 2, 1), np.matmul(F, x1)).flatten() # [2000,]
    Fx1 = np.matmul(F, x1).reshape(-1, 3) # [2000,3]
    Ftx2 = np.matmul(F.transpose(0, 2, 1), x2).reshape(-1, 3) # [2000,3]
