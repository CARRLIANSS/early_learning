...
from dump_match.feature_match import computeNN
from dump_match.utils import saveh5, loadh5
from dump_match.geom import load_geom, parse_geom, get_episym
...
class Sequence(object):
    def __init__(self, dataset_path, dump_dir, desc_name, vis_th, pair_num, pair_name=None):
        ...
        self.geom_fullpath_list = self.parse_list_file(self.data_path, geom_list_file) # calibration
        self.vis_fullpath_list = self.parse_list_file(self.data_path, vis_list_file) # visibility
        # load geom and vis
        self.geom, self.vis = [], []
        ...
        if pair_name is None:
            self.pairs = []
            for ii, jj in itertools.product(xrange(img_num), xrange(img_num)): # index pair by pair
    ...
    def dump_nn(self, ii, jj):
        ...
        if not os.path.exists(dump_file):
            ...
            # Dump to disk
            dump_dict = {}
            dump_dict["idx_sort"] = idx_sort # index of matching keypoint
            dump_dict["ratio_test"] = ratio_test # matching probability
            dump_dict["mutual_nearest"] = mutual_nearest # Mutual Nearest Neighbor Judgment
    ...
    def unpack_K(self, geom):
        ...
        fx = K[0, 0]
        fy = K[1, 1]
        return cx, cy, [fx, fy] # element in K/intrinsics
    ...
    def make_xy(self, ii, jj):
        geom_i, geom_j = parse_geom(self.geom[ii]), parse_geom(self.geom[jj]) # intrinsics and extrinsics
        # should check the image size here
        # load img and check img_size
        image_i, image_j = self.image_fullpath_list[ii], self.image_fullpath_list[jj] # image path

        # keypoint coordinate
        kp_i = loadh5(image_i + '.' + self.desc_name + '.hdf5')["keypoints"][:, :2] # keypoint coordinate of image_i
        kp_j = loadh5(image_j + '.' + self.desc_name + '.hdf5')["keypoints"][:, :2] # keypoint coordinate of image_j

        # intrinsics/K matrix
        cx1, cy1, f1 = self.unpack_K(geom_i) # element in K/intrinsics of image_i
        cx2, cy2, f2 = self.unpack_K(geom_j) # element in K/intrinsics of image_i
        # keypoint coordinate processing
        x1 = self.norm_kp(cx1, cy1, f1[0], f1[1], kp_i) # normalization of keypoint coordinate
        ...
        # keypoint pairs
        x2 = x2[idx_sort[1], :] # Reorder according to matching
        xs = np.concatenate([x1, x2], axis=1).reshape(1, -1, 4) # [2000,2] * 2 -> [1,2000,4]
            ...
            for pair_idx, pair in enumerate(self.pairs):
                ...
                if len(res) != 0:
                    for var_idx, name in enumerate(var_name):
                        res_dict[name] += [res[var_idx]] # add all pairs
            for name in var_name:
