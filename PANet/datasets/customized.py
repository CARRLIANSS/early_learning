...
import torch
import numpy as np
from datasets.pascal import VOC
from datasets.coco import COCOSeg
from datasets.common import PairedDataset
...
def getMask(label, scribble, class_id, class_ids):
    ...
    for class_id in class_ids:
        bg_mask[label == class_id] = 0 # 将episode中其他类制为前景

    # Scribble Mask
    bg_scribble = scribble == 0
    # bg_scribble_np = bg_scribble.numpy()
    fg_scribble = torch.where((fg_mask == 1)
                              & (scribble != 0)
                              & (scribble != 255),
                              scribble, torch.zeros_like(fg_mask))
    # fg_scribble_np = fg_scribble.numpy()
